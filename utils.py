from typing import Dict, Iterable, List, Optional, Tuple, Union, Type , TypeVar 
import torch as th 
import numpy as np 
import random 
...
def obs_as_tensor(
    ...
    if isinstance(obs, np.ndarray):
        ...
        return {key: th.as_tensor(_obs,dtype=th.float32).to(device) for (key, _obs) in obs.items()}
    else:
